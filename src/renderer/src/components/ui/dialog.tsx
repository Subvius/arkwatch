...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-[95vw] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-lg border bg-[hsl(var(--panel))] p-6 shadow-lg',
        className
      )}
      {...props}
    >
      {children}
      <DialogClose className="absolute right-4 top-4 rounded-sm text-[hsl(var(--muted))] hover:text-[hsl(var(--ink))] focus:outline-none">
        <X className="h-4 w-4" />
